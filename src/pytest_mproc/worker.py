...
from pytest_mproc import user_output, _get_my_ip, _find_free_port
from pytest_mproc.auth import get_auth_key
...
def main(test_q: multiprocessing.JoinableQueue, results_q: multiprocessing.JoinableQueue,
         args: List[str], worker_id: str):
    ...
    try:
        ...
        with suppress(Exception):  # if other end died, result_q is gone, so suppress exceptions
            results_q.put(ClientDied(os.getpid(), _get_my_ip(), errored=True, message=msg))
    finally:
        signal.alarm(120)
        results_q.join()
        results_q.close()
